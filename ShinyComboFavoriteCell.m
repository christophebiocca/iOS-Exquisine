...
@implementation ShinyComboFavoriteCell
...
+(BOOL) canDisplayData:(id)data
{
    ...
    return ([data isKindOfClass:[NSDictionary class]] && [data objectForKey:@"favoriteCellCombo"]);
}

+(NSString *) cellIdentifier
{
    //Must return a unique string identifier for this type of cell.
    return @"ShinyComboFavoriteCell";
}
...
-(void) setData:(id) data
{
    ...
    {
        CLLog(LOG_LEVEL_ERROR, @"An unsupported data type was sent to ShinyComboFavoriteCell's setData:");
        return;
    }
    
    theCombo = [data objectForKey:@"favoriteCellCombo"];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateCell) name:COMBO_MODIFIED object:theCombo];
...
-(void)wasClicked
{
    [[AppData appData] toggleFavoriteCombo:theCombo];
...
-(void) updateCell
{
    ...
    if ([[AppData appData] isFavoriteCombo:theCombo]) {
